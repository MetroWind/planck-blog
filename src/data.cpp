...
{
mw::E<Post>
postFromRow(const std::tuple<int, int, std::string, std::string, std::string,
                             int64_t, int64_t, std::string, std::string>& row)
{
    int markup = std::get<1>(row);
    if(!Post::isValidMarkupInt(markup))
    {
        return std::unexpected(
            mw::runtimeError(std::format("Invalid markup: {}", markup)));
    }
    Post p;
    p.id = std::get<0>(row);
...
mw::E<nlohmann::json>
DataSourceInterface::getValueWithDefault(const std::string& key,
...
DataSourceSqlite::fromFile(const std::string& db_file)
{
    ...
    ASSIGN_OR_RETURN(int64_t v, data_source->getSchemaVersion());
    if(v == 1)
    {
...
mw::E<std::optional<Post>> DataSourceSqlite::getPost(int64_t id) const
{
    ASSIGN_OR_RETURN(
        std::vector<Post> ps,
        filterPosts(std::format("WHERE publish_time != 0 AND id = {}", id)));
    if(ps.empty())
    {
        return std::nullopt;
    }
...
mw::E<void> DataSourceSqlite::updatePost(Post&& new_post) const
{
    ...
    {
        return std::unexpected(
            mw::runtimeError("Trying to update a post without ID"));
    }
    ASSIGN_OR_RETURN(
        auto sql,
        db->statementFromStr("UPDATE Posts SET markup = ?, title = ?, abstract "
                             "= ?, content = ?, "
                             "update_time = ?, language = ? WHERE id = ?;"));
    DO_OR_RETURN(sql.bind(static_cast<int>(new_post.markup), new_post.title,
                          new_post.abstract, new_post.raw_content,
                          mw::timeToSeconds(mw::Clock::now()),
                          new_post.language, *new_post.id));
...
mw::E<void> DataSourceSqlite::updatePostNoUpdateTime(const Post& new_post) const
{
    ...
    ASSIGN_OR_RETURN(auto sql,
                     db->statementFromStr("UPDATE Posts SET markup = ?, title "
                                          "= ?, abstract = ?, content = ?, "
                                          "language = ? WHERE id = ?;"));
    DO_OR_RETURN(sql.bind(static_cast<int>(new_post.markup), new_post.title,
                          new_post.abstract, new_post.raw_content,
...
mw::E<int64_t> DataSourceSqlite::saveDraft(Post&& new_draft) const
{
    ...
    {
        return std::unexpected(
            mw::runtimeError("New draft should not have ID"));
    }
    ASSIGN_OR_RETURN(auto sql,
                     db->statementFromStr("INSERT INTO Posts (markup, title, "
                                          "abstract, content, language, author)"
                                          " VALUES (?, ?, ?, ?, ?, ?)"));
    DO_OR_RETURN(sql.bind(static_cast<int>(new_draft.markup), new_draft.title,
                          new_draft.abstract, new_draft.raw_content,
                          new_draft.language, new_draft.author));
    DO_OR_RETURN(db->execute(std::move(sql)));
...
mw::E<void> DataSourceSqlite::editDraft(const Post& draft) const
{
    ...
    {
        return std::unexpected(
            mw::runtimeError("Trying to edit a draft without ID"));
    }
    ASSIGN_OR_RETURN(auto sql,
                     db->statementFromStr(
                         "UPDATE Posts SET markup = ?, title = ?, abstract = "
                         "?, content = ?, "
                         "language = ? WHERE id = ? AND publish_time = 0;"));
    DO_OR_RETURN(sql.bind(static_cast<int>(draft.markup), draft.title,
                          draft.abstract, draft.raw_content, draft.language,
                          *draft.id));
    DO_OR_RETURN(db->execute(std::move(sql)));
...
{
    ASSIGN_OR_RETURN(
        auto sql, db->statementFromStr("UPDATE Posts SET publish_time = ? "
                                       "WHERE id = ? AND publish_time = 0;"));
...
{
    ASSIGN_OR_RETURN(auto sql,
                     db->statementFromStr("DELETE FROM Posts  WHERE id = ?;"));
...
{
    ASSIGN_OR_RETURN(
        mw::SQLiteStatement sql,
        db->statementFromStr("INSERT OR IGNORE INTO Attachments "
                             "(original_name, hash, upload_time,"
                             " content_type) VALUES (?, ?, ?, ?);"));
...
mw::E<std::vector<Attachment>> DataSourceSqlite::getAttachments() const
{
    ASSIGN_OR_RETURN(
        auto rows,
        (db->eval<std::string, std::string, int64_t, std::string>(
    ...
    {
        result.emplace_back(std::get<0>(row), std::get<1>(row),
...
mw::E<void> DataSourceSqlite::deleteAttachment(const std::string& hash) const
{
    ...
    {
        return std::unexpected(
            mw::runtimeError("Failed to delete attachment."));
    }
    return {};
}
...
DataSourceSqlite::getReferralsOfAttachment(const std::string& hash) const
{
    ...
    ASSIGN_OR_RETURN(auto rows,
                     (db->eval<std::string, int64_t>(std::move(sql))));
...
mw::E<void>
DataSourceSqlite::addAttachmentReferral(const std::string& attachment_hash,
                                        const std::string& url) const
{
    ASSIGN_OR_RETURN(
        mw::SQLiteStatement sql,
        db->statementFromStr("SELECT request_count FROM AttachmentReferrals "
    ...
    else
    {
        ASSIGN_OR_RETURN(
            mw::SQLiteStatement sql,
            db->statementFromStr("UPDATE AttachmentReferrals SET request_count "
                                 "= request_count + 1 "
                                 "WHERE hash = ? AND origin = ?;"));
        DO_OR_RETURN(sql.bind(attachment_hash, url));
        return db->execute(std::move(sql));
...
mw::E<void>
DataSourceSqlite::forceSetPostTimes(int64_t id, const mw::Time& publish,
...
DataSourceSqlite::filterPosts(std::string_view sql_suffix) const
{
    ASSIGN_OR_RETURN(
        auto sql,
        db->statementFromStr(std::format(
            "SELECT id, markup, title, abstract, content, publish_time, "
            "update_time, language, author FROM Posts {};",
            sql_suffix)));
    ASSIGN_OR_RETURN(
        auto rows,
        (db->eval<int, int, std::string, std::string, std::string, int64_t,
...
DataSourceSqlite::getValue(const std::string& key) const
{
    ASSIGN_OR_RETURN(
        auto sql,
        db->statementFromStr("SELECT value FROM KeyValues WHERE key = ?;"));
...
mw::E<int64_t>
DataSourceSqlite::insertWebMention(const WebMention& mention) const
{
    ASSIGN_OR_RETURN(
        auto sql,
        db->statementFromStr(
            "INSERT INTO WebMentions (source, target_id, status, author_name, "
            "author_photo, content, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)"));
    DO_OR_RETURN(sql.bind(mention.source, mention.target_id, mention.status,
                          mention.author_name, mention.author_photo,
                          mention.content, mention.created_at));
    DO_OR_RETURN(db->execute(std::move(sql)));
    return db->lastInsertRowID();
}

mw::E<void>
DataSourceSqlite::updateWebMention(int64_t id, const std::string& status,
                                   std::optional<std::string> author_name,
                                   std::optional<std::string> author_photo,
                                   std::optional<std::string> content) const
{
    ASSIGN_OR_RETURN(auto sql,
                     db->statementFromStr("UPDATE WebMentions SET status = ?, "
    ...
    if(rows_count == 0)
    {
        return std::unexpected(mw::runtimeError("WebMention not found"));
    }
    return {};
}
...
DataSourceSqlite::getVerifiedWebMentionsForPost(int64_t postId) const
{
    ASSIGN_OR_RETURN(
        auto sql,
        db->statementFromStr(
            "SELECT id, source, target_id, status, author_name, author_photo, "
            "content, created_at FROM WebMentions WHERE target_id = ? AND "
            "status = 'verified' ORDER BY created_at ASC;"));
    DO_OR_RETURN(sql.bind(postId));
    ASSIGN_OR_RETURN(
        auto rows,
        (db->eval<int64_t, std::string, int64_t, std::string,
    ...
    for(auto& row : rows)
    {
        ...
        m.status = std::move(std::get<3>(row));
